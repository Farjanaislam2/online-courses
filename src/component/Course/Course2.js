import React from "react";

const Courde2 = () => {
  return (
    <div>
      <h1
        className="text-3xl text-black font-bold text-center
            mt-10"
      >
        Code
      </h1>

      <div className="flex gap-1 mt-10 justify-evenly bg-white text-white" style={{gap:'1px'}}>
        <div
          className="p-3 text-center justify-center lg:flex hidden hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1 className="">Php</h1>
        </div>
        <div
          className="columns-1 justify-center p-3 lg:flex hidden text-center hover:font-bold w-full "
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Node.js</h1>
        </div>
        <div
          className="p-3 text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Java</h1>
        </div>
        <div
          className="p-3 lg:flex justify-center hidden text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Flutter</h1>
        </div>
        <div
          className="p-3 text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>React.js</h1>
        </div>
        <div
          className="p-3 lg:flex justify-center hidden text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Html</h1>
        </div>
        <div
          className="p-3 text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Flutter</h1>
        </div>
        <div
          className="p-3 lg:flex  hidden text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Wordpress</h1>
        </div>
        <div
          className="p-3 text-center hover:font-bold w-full"
          style={{ backgroundColor: "#530A0A" }}
        >
          <h1>Mongo</h1>
        </div>
      </div>
    </div>
  );
};

export default Courde2;
